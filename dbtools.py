...
class Dao(object):
    ...
    def find_all(self, orderKey):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY {} ASC'.format(self._table_name, orderKey))
    ...
    def update(self,**keyvals):
        ...
        stmt = 'UPDATE {} SET {} WHERE {}'\
               .format(self._table_name, column_names[0]+'=?',' AND '.join([col + '=?' for col in column_names[1:]]))
        self._conn.cursor().execute(stmt, params)